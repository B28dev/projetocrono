...
function SimButton({ label, color = TOKEN.pink, onClick, disabled = false }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="shrink-0 rounded-lg border px-4 py-2 font-mono text-[10px] font-bold uppercase tracking-wider transition-all duration-200"
      style={{
        background: disabled ? 'rgba(255,255,255,0.03)' : `${color}15`,
        borderColor: disabled ? 'rgba(255,255,255,0.06)' : `${color}40`,
        color: disabled ? TOKEN.muted : color,
...
function MissionItemRow({ item, contentItems }) {
  const content = contentItems.find((c) => c.id === item.contentItemId);
  const statusColors = {
    pending:       TOKEN.muted,
    completed:     '#34d399',
    skipped:       '#f87171',
    revealed_only: '#fbbf24',
  };
  const originIcon = { today: '☀️', backlog: '📦', reinforcement: '🔄' };

  return (
    <div className="group flex items-start gap-4 border-b border-white/[0.03] py-3.5 transition-colors hover:bg-white/[0.01] px-2 rounded-lg">
      <span className="mt-0.5 w-6 shrink-0 text-center text-sm opacity-80 mix-blend-luminosity">
        {originIcon[item.origin] ?? '—'}
      </span>
      <div className="min-w-0 flex-1">
        <p className="mb-0.5 text-sm font-medium text-zinc-200 leading-snug">
          {content?.title ?? item.contentItemId}
        </p>
        <p className="text-[11px] text-zinc-500 capitalize">
          {content?.kind?.replace('_', ' ') ?? '—'} · {item.origin} {item.requiredForCleanDay ? '· Obrigatório' : ''}
        </p>
      </div>
      <span
        className="shrink-0 font-mono text-[10px] uppercase tracking-widest font-bold"
        style={{ color: statusColors[item.status] ?? TOKEN.muted }}
      >
        {item.status}
      </span>
    </div>
...
const DashboardSidebar = memo(function DashboardSidebar() {
  return (
    <aside className="hidden lg:flex w-[260px] flex-col border-r border-white/[0.04] bg-[#0A0A12] shadow-2xl relative z-40">
      <div className="flex flex-col h-full sticky top-0">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2 scrollbar-thin scrollbar-thumb-white/10">
          ...
          <nav className="space-y-1">
            ...
            <SidebarNavItem icon="🎯" label="Missão Diária" />
            <SidebarNavItem icon="📚" label="Disciplinas" />
            <SidebarNavItem icon="⚔️" label="Arena" />
            <SidebarNavItem icon="💎" label="Recompensas" />
          </nav>
        </div>

        {/* Footer */}
        <div className="border-t border-white/[0.04] px-4 py-6">
          <SidebarNavItem icon="⚙️" label="Configurações" />
...
function DashboardTopbar({ momentum, dateLabel }) {
  return (
    <header className="sticky top-0 z-30 flex items-center justify-between border-b border-white/[0.03] bg-[#0A0A12]/80 px-6 py-4 backdrop-blur-xl lg:px-10 lg:py-5 shadow-sm">
  ...
  return (
    <div className="flex min-h-screen w-full bg-[#05050A] font-inter text-zinc-100 overflow-hidden">
      ...
      <main className="relative flex min-h-screen flex-1 flex-col overflow-y-auto overflow-x-hidden">
        ...
        {/* ── TOPBAR ── */}
        <DashboardTopbar momentum={momentum} dateLabel={getLocalDateString()} />
        ...
        <div className="relative z-10 w-full max-w-[1600px] flex-1 px-4 py-6 md:px-8 md:py-8 lg:p-10 mx-auto">
          ...
          <div ref={gridRef} className="space-y-6 lg:space-y-8">
            ...
            {/* LINHA 1: Cards compactos estratégicos */}
            ...
            {/* LINHA 2: Bloco largo Resumo Operacional (Estados Derivados) */}
            ...
            {/* MAIN GRID BLOCK for Row 3 & 4 */}
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
              
              {/* LINHA 3 (Esquerda): Missão do Hoje (Peça Central) */}
                  ...
                  <div className="border-b border-white/[0.04] px-6 py-5 lg:px-8 lg:py-6">
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        ...
                        <h2 className="font-display text-xl lg:text-2xl font-bold tracking-tight text-white mb-2">
                          Plano de Batalha de Hoje
                        </h2>
                        <p className="text-sm text-zinc-400">
                          {todayMission?.targetValidations} validações reais • {todayMission?.targetBlocks} blocos programados
                        </p>
                      </div>
                      <div className="hidden sm:flex h-12 w-12 items-center justify-center rounded-xl bg-white/[0.02] border border-white/[0.05] shadow-inner text-xl">
                        🎯
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="px-4 py-4 lg:px-6 lg:py-6">
                    {loadingState === 'empty' || todayItems.length === 0 ? (
                      ...
                      <div className="flex flex-col gap-1">
                        {todayItems.map((item) => (
                          <MissionItemRow key={item.id} item={item} contentItems={contentItems} />
                        ))}
                      </div>
                    )}
                  </div>
              ...
              {/* LINHA 4 (Direita): Painel Discreto de Simulação */}
              <div className="lg:col-span-4 flex flex-col">
                <div className="lab-card rounded-2xl border border-pink-500/10 bg-pink-500/[0.02] p-6 lg:p-8 backdrop-blur-xl">
                  <div className="flex items-center gap-3 mb-6">
                    ...
                    <h3 className="font-display text-lg font-bold text-pink-400/90 tracking-tight">Crono Engine Test</h3>
                  </div>
                  <p className="mb-6 text-xs text-zinc-500 leading-relaxed font-medium">
                    Estes controles injetam eventos diretamente na <span className="font-mono bg-white/[0.05] px-1 rounded">ProgressStore</span> simulando o engine. Serão removidos na interface final.
                  </p>
                  <div className="flex flex-col gap-3">
                    <SimButton label="+ Executar Validação Real" color={TOKEN.cyan} onClick={handleSimulateValidation} />
                    <SimButton label="Simular Quebra de Streak" color="#f87171" onClick={handleSimulateBreakStreak} />
                    <SimButton label="Injetar Dívida no Backlog" color="#fbbf24" onClick={handleSimulateBacklog} />
                    <div className="mt-4 pt-4 border-t border-white/[0.04]">
                      <SimButton label="Resetar Servidor Local" color={TOKEN.muted} onClick={handleReset} />
                    </div>
                  </div>
                </div>
              </div>
