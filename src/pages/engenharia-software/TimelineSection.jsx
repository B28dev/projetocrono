    ...
    <article className="rounded-xl border border-white/[0.08] bg-white/[0.018] p-4 backdrop-blur-xl dark:border-stone-300 dark:bg-white/80">
      <div className="flex flex-col gap-4">
        <div className="max-w-3xl">
          <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-cyan-300 dark:text-cyan-700">linha do plano</p>
          <h3 className="mt-1 text-lg font-semibold text-white dark:text-stone-950">{timeline.title}</h3>
        </div>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="inline-flex min-h-10 items-center justify-center rounded-lg border border-white/10 bg-white/[0.03] px-4 py-2.5 text-sm font-semibold text-zinc-300 transition-colors hover:text-white dark:border-stone-300 dark:bg-stone-100 dark:text-stone-700 dark:hover:text-stone-950"
            aria-expanded="false"
          >
            Ver janela
          </button>
            ...
            <article key={item.id} className="rounded-lg border border-white/[0.08] bg-white/[0.025] px-3.5 py-3 dark:border-stone-200 dark:bg-stone-50">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div>
                  ...
                  <p className="mt-1 text-xs font-medium text-zinc-400 dark:text-stone-600">{item.date}</p>
                </div>
              ...
              {isExpanded ? (
                <div id={`timeline-event-${item.id}`} className="mt-3 border-t border-white/[0.08] pt-3 dark:border-stone-200">
                  <p className="text-sm font-medium leading-relaxed text-zinc-300 dark:text-stone-700">{item.summary}</p>
                  <p className="mt-1 text-xs text-zinc-400 dark:text-stone-600">{item.detail.body}</p>
                </div>
              ) : null}
            </article>
          );
