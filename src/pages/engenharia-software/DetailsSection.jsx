...
function SubjectCards({ items }) {
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {items.map((subject) => (
        <article key={subject.id} className="rounded-[20px] border border-white/[0.06] bg-white/[0.03] p-4 dark:border-stone-200 dark:bg-stone-50">
          <div className="flex items-center justify-between gap-3">
            <span className="rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-[10px] font-mono uppercase tracking-[0.18em] text-white/55 dark:border-stone-300 dark:bg-white dark:text-stone-500">
              {subject.shortTitle}
            </span>
            <span className="text-[10px] font-mono uppercase tracking-[0.18em] text-cyan-300 dark:text-cyan-700">
          ...
          <p className="mt-3 text-xs leading-relaxed text-zinc-500 dark:text-stone-600">{subject.topicLabel}</p>
        </article>
      ))}
    </div>
  );
...
function ResourcesSection({ resources }) {
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <div className="space-y-3">
        <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-cyan-300 dark:text-cyan-700">playlists</p>
        {resources.playlists.map((playlist) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="block rounded-[18px] border border-white/[0.06] bg-white/[0.03] p-4 transition-colors hover:border-cyan-400/20 hover:bg-white/[0.05] dark:border-stone-200 dark:bg-stone-50"
          >
      ...
      <div className="space-y-3">
        <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-fuchsia-300 dark:text-fuchsia-700">vídeos por tópico</p>
        {resources.videosByTopic.map((set) => (
          <div key={set.id} className="rounded-[18px] border border-white/[0.06] bg-white/[0.03] p-4 dark:border-stone-200 dark:bg-stone-50">
            ...
            <div className="mt-3 space-y-2">
              {set.videos.slice(0, 3).map((video, index) => (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex min-h-11 items-center justify-between gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-xs text-zinc-300 transition-colors hover:text-white dark:border-stone-200 dark:bg-white dark:text-stone-700"
                >
                  <span className="truncate">{video.title}</span>
                  <span className="shrink-0 text-[10px] uppercase tracking-[0.18em] text-zinc-500 dark:text-stone-500">video</span>
                </a>
              ))}
            </div>
          </div>
...
function SummariesSection({ summaries }) {
  return (
    <div className="grid gap-4 xl:grid-cols-2">
      <div>
        <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.22em] text-cyan-300 dark:text-cyan-700">resumos modelo</p>
        <SummaryAccordion summaries={summaries.modelSummaries} />
      </div>
      <div>
        <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.22em] text-fuchsia-300 dark:text-fuchsia-700">o que as provas cobraram</p>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      {exercises.blocks.map((block) => (
        <article key={block.id} className="rounded-[20px] border border-white/[0.06] bg-white/[0.03] p-4 dark:border-stone-200 dark:bg-stone-50">
          ...
          <div className="mt-4 grid gap-2">
            <div className="rounded-xl border border-white/10 bg-white/[0.03] px-3 py-3 text-xs text-zinc-300 dark:border-stone-200 dark:bg-white dark:text-stone-700">
...
export default function DetailsSection({ details }) {
  return (
    <section id={details.id} className="space-y-5 scroll-mt-24" aria-label={details.title}>
