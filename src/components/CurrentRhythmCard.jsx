...
function CurrentRhythmCard({
  ...
  const temHoje = todayTotalCount > 0;
  const isSuccess = temHoje && todayPendingCount === 0;
  ...
  const cardStyleClass = isSuccess
    ? isCyber
      ? 'border-emerald-400/40 bg-emerald-400/10 shadow-[0_0_20px_rgba(16,185,129,0.16)]'
      : isLight
    ...
    : 'border-blue-500/25 bg-blue-500/8 dark:border-stone-300 dark:bg-white/80 cyberpunk:border-[#00e8ff]/25 cyberpunk:bg-[#00e8ff]/8';

  const labelStyleClass = isSuccess
    ? isCyber
      ? 'text-emerald-300'
      : isLight
      ? 'text-emerald-700'
      : 'text-emerald-300'
    : 'text-zinc-500 dark:text-stone-600 cyberpunk:text-[#00e8ff]';

  const valueStyleClass = isSuccess
    ? isCyber
      ? 'text-emerald-200'
      : isLight
      ? 'text-emerald-700'
  ...
  return (
    <div className={`rounded-lg border px-2.5 py-2 backdrop-blur-md transition-all duration-500 ${cardStyleClass}`}>
      <div className="flex items-start justify-between gap-3">
        <p className={`text-[10px] uppercase tracking-widest cyberpunk:font-mono ${labelStyleClass}`}>
          Ritmo atual
        </p>
