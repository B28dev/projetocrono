...
export default function MethodTooltip({ method }) {
  ...
  return (
    <div className="flex items-start gap-3 rounded-xl border border-zinc-700/60 bg-surface-2 px-4 py-3 animate-[animationIn_0.4s_ease-out_both] dark:border-stone-300 dark:bg-white/80">
      <span className="text-xl flex-shrink-0 mt-0.5">{info.icon}</span>
      <div>
        <p className="text-sm font-semibold text-zinc-100 mb-0.5 dark:text-stone-900">{info.label}</p>
        <p className="text-xs text-zinc-400 leading-relaxed dark:text-stone-600">{info.description}</p>
      </div>
    </div>
  );
}
